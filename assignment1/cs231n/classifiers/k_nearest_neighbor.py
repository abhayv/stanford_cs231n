...
class KNearestNeighbor:
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      a = np.array([X[i]])
      b = self.X_train
      ab_neg2 = -2*np.array((np.mat(b)*np.mat(a.transpose()))).transpose()
      b_sqr = np.sum(np.square(b), axis=1)
      dists[i] = b_sqr + ab_neg2  + np.sum(np.square(a))
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    ab_2 = -(2*(np.mat(X)*np.mat(self.X_train.transpose()))).transpose()
    b_sqr = np.sum(np.square(self.X_train),axis=1)
    dists = (np.array((b_sqr + ab_2.transpose()).transpose() + a_sqr)).transpose()
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
